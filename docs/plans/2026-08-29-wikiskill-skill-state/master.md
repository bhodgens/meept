...
| gap-closure | COMPLETE | 2026-08-30 commits a8ba32f0 + 3bdfda86 + 2b5e0985 + 2b5e0985-docs — (1) state-run traces wired via NewTraceStoreWriter payload seam; (2) loop turn-level trace writer wired (had ZERO prod callers); (3) registry specialist loops wired (SetTraceWriter/SetUsageTracker/SetLearningPipeline) — dispatcher-routed turns now reach the stores, live-verified trace on disk; (4) LoadJSON5 duration preprocessing added + pre-existing string-mangling regex bug fixed with guard test. All -race green |
