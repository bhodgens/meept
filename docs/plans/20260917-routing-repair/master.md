...
| 05 | COMPLETE (verified) | Evidence ledger in verification.md (every finding dispositioned); FINAL-REPORT CORRECTIONS 6-8 appended; selection-on-test caveats added; intent-routing.md reconciled to current source (I10b SUPERSEDED by de671eb2); classifier-evaluation.md created; campaign design frozen, not run. Parent gates: full internal/agent suite ok, vet clean, both self-tests PASS. |
| LIVE | VERIFIED_LIVE (partial) | Two full acceptance passes on the scratch rig (16 attempts, judge attached, evidence available on all). Stable routing PASS: AR-1, AR-2, AR-3, arithmetic-control. git-status-control flipped to committer/git/llm in pass 2 after gitSubcommandRe fix (agreement veto now admits 'git <subcommand>' pairs). Remaining FAILs are 8B model territory: write-vs-code lane preference on file creation (3 cases, stable) and 0/8 outcomes from tool-emission gaps (staged pending writes; announced-action guard fired live converting fake success to visible annotation). Media control stays BLOCKED (producer evidence). Full ledger: docs/plans/20260917-routing-repair/verification.md |
