...
|-------|--------|------------|-------------|
| 01-spec-config.md | COMPLETE | 0 | Commit 178cd2b3. Field tags exactly per Contract 1; BOTH conversion sites wired (definitionToSpec + mergeSpec) — the Verification-treatment deviation was NOT taken; mergeSpec preserves the field on re-load (D16 two-half pattern). No shipped AGENT.md sets the field (D14). Docs in docs/configuration/agents.md. |
| 02-hook-pipeline.md | COMPLETE | 0 | Commit 53e0dd48. Seam: reasoningCycle per-iteration, after GetWindowedMessages, before override/alias-resolution + LLM call (documented in code with file:line evidence). ExtraMessages one-shot prepend + ModelOverride via SetModelOverride; hook panics recovered + logged; zero-value mod true no-op; no-hooks regression tested. Flagged side effect per master Notes: security/secret PrepareNextTurn hooks (components wiring) are now live. |
| 03-hook-wiring.md | COMPLETE | 0 | Commit c88cc545. DecideEscalation/ModelResolver/EscalationDecision match Contract 3 exactly (ResolveEscalationRef per audit B2). Legacy path byte-identical (format string + Reason "verification escalation" pinned by tests); Escalate path stores h.pendingEscalation, resets fixCount (Q2 comment). Nil-guarded SetAgentSpec/SetResolver. All analyzers clean. |
| 04-resolution-lifecycle.md | COMPLETE | 1 | Commit a00b76a5. ResolveEscalationRef (alias→ResolveForAlias inherits rotation/cooldown/quota; direct→ResolveModelRef), routing-log reason "escalation" resolver-side. R1 honored: SetPersistentModelOverride for the full escalated budget, cleared on fresh turn via ModelOverrideFreshTurnHook swept at RunOnceWithParts start (no sticky escalation — tested). Bus topic agent.model_escalated → agent_progress (server.go case + AGENTS.md invariant line); make graphs regenerated + committed. ORCHESTRATOR FIX-UPS (1 review cycle, small-gap fixes per dispatch rules): from_model now reports the pre-escalation base ref and fix_loops the effective floor (Contract 4 payload semantics); two `_ =` routing-log sites rewritten log-and-continue (§2 no-ignored-errors). Deviations: ApplyEscalation returns bool consumed-or-not (master sketch showed no return; call sites use it); SetModelOverride gained a same-ref anti-demotion guard so applyTurnModification's one-shot replay cannot clear the persistent escalation override. |
