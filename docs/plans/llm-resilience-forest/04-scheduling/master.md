...
| 02-queue-priority.md | COMPLETE | 0 | COMMITTED d7d2cddc (schema/claims/migration) + 35c79887 (stamping + R4 plumbing). All four claim ORDER BY sites lead `interactive DESC` (affinity CASE preserved under it, regression-tested); idx_jobs_claim in migrate() AFTER column migration (baseSchema placement broke pre-interactive DBs — CREATE TABLE is a no-op there); ListByTaskID SELECT gained interactive for scan parity; QueueStats.ByInteractive added. StampInteractive in services (queue stays session-free, layering note). R4: StepJobPayload.SessionID + task_steps.session_id persisted; scheduleStep resolves via TaskID→GetLinkedSessions, retries re-stamp identically; one_off/system false by construction; stamped-at-enqueue semantics documented in job-scheduling.md. ANOMALY: d7d2cddc (my exact intended Task 1+2 commit) appeared mid-leaf without orchestrator action — content reviewed sound before completing; remainder committed under 35c79887. |
