// Package gate implements command-level quality gates: a shell check that
// must pass before work may be reported as complete.
//
// This package is the neutral home of RunGate. It was extracted from
// internal/employee so that the agent loop (internal/agent) can reuse the
// SAME gate implementation for roster AGENT.md gates without importing
// internal/employee, which transitively drags in bot → comm/http → agent and
// creates an import cycle. internal/employee re-exports these symbols via
// type/var aliases, so all employee-side callers are unaffected.
//
// This is ORTHOGONAL to the employee enforcement engine (enforcement.go):
// enforcement polices individual actions pre-execution; the quality gate
// decides whether work may be marked complete after execution. Do not
// conflate them.
package gate
...
// the goal).
//
//nolint:revive // package-qualified name (gate.GateConfig) is intentional; package name matches domain
...
//
//nolint:revive // package-qualified name (gate.GateResult) is intentional; package name matches domain
...
//
//nolint:revive // package-qualified name (gate.GateState) is intentional; package name matches domain
...
// truncate caps s at maxLen bytes, appending an elision marker.
