...
func seedCommandPatterns() []SeedCommandPattern {
	return []SeedCommandPattern{
		...
		{Pattern: `\brm\s+-rf\s+/\s*$`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Recursive delete from root", Immutable: true},
		{Pattern: `\bmkfs\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Filesystem format", Immutable: true},
		{Pattern: `\bdd\s+if=/dev/(zero|urandom)\s+of=/dev/`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Disk overwrite", Immutable: true},
		{Pattern: `:\(\)\s*\{\s*:\s*\|\s*:\s*&\s*\}\s*;\s*:`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Fork bomb", Immutable: true},
		{Pattern: `\b(shutdown|reboot|halt|poweroff)\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "System power control", Immutable: true},
		{Pattern: `\binit\s+[06]\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Runlevel change", Immutable: true},
		{Pattern: `\b>\s*/dev/sd[a-z]`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "Direct device write", Immutable: true},
		{Pattern: `\bchmod\s+777\s+/`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategoryDestructive, Description: "World-writable root permissions", Immutable: true},

		// Self-replication detection
		{Pattern: `\bgit\s+clone\b.*meept`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySelfReplication, Description: "Cloning meept repository", Immutable: true},
		{Pattern: `\bpip\s+install\b.*meept`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySelfReplication, Description: "Installing meept package", Immutable: true},
		{Pattern: `\bcp\b.*meept.*meept`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySelfReplication, Description: "Copying meept to create duplicate", Immutable: true},
		{Pattern: `\bdocker\b.*meept`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySelfReplication, Description: "Docker operation involving meept", Immutable: true},

		// Additional critical patterns
		{Pattern: `\bkernelctl\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySystem, Description: "Kernel control", Immutable: true},
		{Pattern: `\bmodprobe\s+-r\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySystem, Description: "Kernel module removal", Immutable: true},
		{Pattern: `\bsysctl\s+-w\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySystem, Description: "Kernel parameter modification", Immutable: true},
		{Pattern: `\b>\s*/etc/passwd\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySystem, Description: "Password file overwrite", Immutable: true},
		{Pattern: `\b>\s*/etc/shadow\b`, PatternType: "regex", RiskLevel: RiskCritical, Category: CategorySystem, Description: "Shadow file overwrite", Immutable: true},

		// HIGH (3) -- Requires confirmation
		{Pattern: `\brm\s+-rf\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryDestructive, Description: "Recursive forced delete", Immutable: false},
		{Pattern: `\brm\s+-r\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryDestructive, Description: "Recursive delete", Immutable: false},
		...
		{Pattern: `\bpip\s+install\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryInstall, Description: "Python package installation", Immutable: false},
		{Pattern: `\bnpm\s+install\s+-g\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryInstall, Description: "Global npm package installation", Immutable: false},
		{Pattern: `\bcargo\s+install\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryInstall, Description: "Rust binary installation", Immutable: false},
		{Pattern: `\b(brew|apt|apt-get|dnf|yum|pacman)\s+install\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryInstall, Description: "System package installation", Immutable: false},
		...
		{Pattern: `\bcurl\s+.*\|\s*(bash|sh|python|python3)\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryCodeExecution, Description: "Pipe-to-shell execution", Immutable: false},
		{Pattern: `\bwget\s+.*\|\s*(bash|sh|python|python3)\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryCodeExecution, Description: "Pipe-to-shell execution", Immutable: false},
		...
		{Pattern: `\bnpm\s+install\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryInstall, Description: "npm package installation", Immutable: false},
		...
		{Pattern: `\bgit\s+push\s+.*--force\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryVCS, Description: "Force push to git", Immutable: false},
		{Pattern: `\bgit\s+reset\s+--hard\b`, PatternType: "regex", RiskLevel: RiskHigh, Category: CategoryVCS, Description: "Hard git reset", Immutable: false},

		// MEDIUM (2) -- Allowed, logged
		{Pattern: `\b(python|python3)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryCodeExecution, Description: "Python interpreter execution", Immutable: false},
		{Pattern: `\b(node|deno|bun)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryCodeExecution, Description: "JavaScript runtime execution", Immutable: false},
		{Pattern: `\bruby\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryCodeExecution, Description: "Ruby interpreter execution", Immutable: false},
		{Pattern: `\bperl\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryCodeExecution, Description: "Perl interpreter execution", Immutable: false},
		{Pattern: `\bmake\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryBuild, Description: "Build system execution", Immutable: false},
		{Pattern: `\bgit\s+(push|reset|rebase|force-push|checkout\s+--)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryVCS, Description: "Destructive git operations", Immutable: false},
		{Pattern: `\bcargo\s+(build|test|run)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryBuild, Description: "Rust build/test/run", Immutable: false},
		...
		{Pattern: `\bgo\s+(build|run|install)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryBuild, Description: "Go build/run/install", Immutable: false},
		{Pattern: `\bzig\s+(build|run)\b`, PatternType: "regex", RiskLevel: RiskMedium, Category: CategoryBuild, Description: "Zig build/run", Immutable: false},

		// LOW (1) -- Allowed, minimal logging
		{Pattern: `\bgit\s+(status|log|diff|branch|show|remote|tag|stash\s+list)\b`, PatternType: "regex", RiskLevel: RiskLow, Category: CategoryVCS, Description: "Read-only git operations", Immutable: false},
