...
func (s *ProjectService) CheckoutBranch(ctx context.Context, id string, branch string) error {
	if s.pm == nil {
	...
	if branch == "" {
		return wrapError("project", "CheckoutBranch", fmt.Errorf("branch name is required"))
	}
