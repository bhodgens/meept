# TriggerRequest

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**Forceomitempty** | Pointer to **bool** |  | [optional] 

## Methods

### NewTriggerRequest

`func NewTriggerRequest() *TriggerRequest`

NewTriggerRequest instantiates a new TriggerRequest object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewTriggerRequestWithDefaults

`func NewTriggerRequestWithDefaults() *TriggerRequest`

NewTriggerRequestWithDefaults instantiates a new TriggerRequest object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetForceomitempty

`func (o *TriggerRequest) GetForceomitempty() bool`

GetForceomitempty returns the Forceomitempty field if non-nil, zero value otherwise.

### GetForceomitemptyOk

`func (o *TriggerRequest) GetForceomitemptyOk() (*bool, bool)`

GetForceomitemptyOk returns a tuple with the Forceomitempty field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetForceomitempty

`func (o *TriggerRequest) SetForceomitempty(v bool)`

SetForceomitempty sets Forceomitempty field to given value.

### HasForceomitempty

`func (o *TriggerRequest) HasForceomitempty() bool`

HasForceomitempty returns a boolean if a field has been set.


[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


