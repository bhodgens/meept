...
func TestBotRPCHandler_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	...
	rpcHandler := bot.NewRPCHandler(mgr)
	handlers := rpcHandler.Handlers()

	// Create via RPC
	def := bot.BotDefinition{
		ID:          "rpc-test",
		Name:        "RPC Test Bot",
		Description: "Test bot via RPC",
		Prompt:      "You are a test bot.",
		Triggers: []bot.BotTrigger{
			{Type: bot.TriggerTypeCron, Schedule: "0 * * * *", Enabled: true},
		},
		MemoryScope: bot.MemoryScopePrivate,
		Constraints: bot.BotConstraints{
			MaxIterations: 3,
	...
	createRaw, _ := json.Marshal(def)
	result, err := handlers["bot.create"](ctx, createRaw)
	if err != nil {
		t.Fatalf("bot.create: %v", err)
	}
	createResp := result.(map[string]any)
	if createResp["status"] != "created" {
		t.Errorf("create status = %v, want %q", createResp["status"], "created")
	}

	// Get via RPC
	getRaw, _ := json.Marshal(map[string]any{"id": "rpc-test"})
	getResult, err := handlers["bot.get"](ctx, getRaw)
	if err != nil {
		t.Fatalf("bot.get: %v", err)
	}
	gotDef := getResult.(*bot.BotDefinition)
	if gotDef.Name != "RPC Test Bot" {
		t.Errorf("Name = %q, want %q", gotDef.Name, "RPC Test Bot")
	}

	// List via RPC
	listRaw, _ := json.Marshal(map[string]any{})
	listResult, _ := handlers["bot.list"](ctx, listRaw)
	listResp := listResult.(map[string]any)
	botsList := listResp["bots"].([]bot.BotDefinition)
	if len(botsList) != 1 {
		t.Errorf("list returned %d bots, want 1", len(botsList))
	}

	// Pause via RPC
	_, err = handlers["bot.pause"](ctx, getRaw)
	if err != nil {
		t.Fatalf("bot.pause: %v", err)
	}

	// Status via RPC
	statusResult, _ := handlers["bot.status"](ctx, getRaw)
	status := statusResult.(*bot.BotState)
	if status.Status != bot.BotStatusStopped {
		t.Errorf("status = %q, want %q", status.Status, bot.BotStatusStopped)
	}

	// Resume via RPC
	_, err = handlers["bot.resume"](ctx, getRaw)
	if err != nil {
		t.Fatalf("bot.resume: %v", err)
	}

	// Delete via RPC
	_, err = handlers["bot.delete"](ctx, getRaw)
	if err != nil {
		t.Fatalf("bot.delete: %v", err)
	}

	// Verify deleted
	_, err = handlers["bot.get"](ctx, getRaw)
	if err == nil {
