...
// TestMergePeerDBIntegration tests the merge logic directly.
func TestMergePeerDBIntegration(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	tmpDir := t.TempDir()

	// Create peer DB with test data
	peerDBPath := filepath.Join(tmpDir, "peer.db")
	peerDB, err := sql.Open("sqlite", peerDBPath)
	if err != nil {
		t.Fatalf("sql.Open (peer) failed: %v", err)
	}
	defer peerDB.Close()

	// Create schema and test data in peer DB
	createTestSchemaAndData(t, peerDB)

	// Create gossip DB
	gossipDBPath := filepath.Join(tmpDir, "gossip.db")
	gossipDB, err := sql.Open("sqlite", gossipDBPath)
	if err != nil {
		t.Fatalf("sql.Open (gossip) failed: %v", err)
	}
	defer gossipDB.Close()

	// Create gossip schema
	_, err = gossipDB.Exec(`
	...
	`)
	if err != nil {
		t.Fatalf("Create gossip schema failed: %v", err)
	}

	// Test: Merge peer DB into gossip DB
	stats, err := backup.MergePeerDB(ctx, gossipDB, peerDBPath, "peer-node")
	if err != nil {
		t.Fatalf("MergePeerDB failed: %v", err)
	}

	if stats.SessionsMerged == 0 {
		t.Error("Expected sessions to be merged")
	}

	// Verify data in gossip DB
	var count int
	err = gossipDB.QueryRow("SELECT COUNT(*) FROM sessions").Scan(&count)
	if err != nil {
		t.Fatalf("Count query failed: %v", err)
	}
	if count == 0 {
		t.Error("Expected sessions in gossip DB after merge")
	}

	// Test: Idempotency - merge again should report 0 new merges
	stats2, err := backup.MergePeerDB(ctx, gossipDB, peerDBPath, "peer-node")
	if err != nil {
		t.Fatalf("Second MergePeerDB failed: %v", err)
	}

	if stats2.SessionsMerged != 0 {
		t.Errorf("Expected 0 sessions on second merge (idempotent), got %d", stats2.SessionsMerged)
	}

	t.Logf("MergePeerDB: sessions=%d, turns=%d, memories=%d",
		stats.SessionsMerged, stats.TurnsMerged, stats.MemoriesMerged)
}
