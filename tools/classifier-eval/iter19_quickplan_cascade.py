...
def build_result(replay_n, a_n, a_ok, b_n, b_ok, c_n, tau,
                 per_intent_expected):
    """Result dict: historical schema exactly (iter-19 never serialized
    misses). Miss identifiers reach stdout only via format_misses_report()."""
    ...
    return {
        ...
        "per_intent_expected": per_intent_expected,
        "tau": round(tau, 3),
    }
...
def main():
    ...
    result = build_result(
        replay_n=tot, a_n=a_n, a_ok=a_ok, b_n=b_n, b_ok=b_ok, c_n=c_n,
        tau=tau,
        per_intent_expected={k: {"n": v[0], "expected_correct": round(v[1], 2)}
                             for k, v in sorted(per_intent.items())})
    outdir = H.RESULTS / "iter-19"
    emit_result(result, outdir, "quickplan-cascade.json")
    print(json.dumps({k: v for k, v in result.items()
                      if k != "per_intent_expected"}, indent=1))
