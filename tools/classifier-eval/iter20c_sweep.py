...
Import-safe (routing-repair leaf 01 privacy contract): model loading,
corpus reads, training, and the embedding-server round trips happen only
inside ``main()``. The per-case cache stores ``eval_harness.case_key``
identifiers, never raw replay input excerpts (the pre-repair version
kept ``input[:60]`` in a ``text`` field).
"""
import sys
...
def cache_entry(input_text, true_intent, a_lab, cue):
    """Per-case cache record with the private excerpt replaced by its
    case key. ``input_text`` is used only to derive the stable
    identifier and is never stored. The probe probability vector is
    attached by the caller under ``p`` (numeric, no text)."""
    return {"true": true_intent,
            "a_lab": a_lab,
            "cue": cue,
...
def main():
    global tok, base, dev, hidden, cases_cache, labs
    ...
    for c, mv, qv in zip(silver, MB_S, QV_S):
        ...
        cc = cache_entry(c["input"], c["expected_intent"], a_lab, has_cue)
        cc["p"] = p.detach().cpu().numpy()
        cases_cache.append(cc)
